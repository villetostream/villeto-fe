...
export interface CreatedPolicyData {
  ...
  scopeDeptForRole: string[];  // dept filter when scope=role ([] = All)
  scopeRoleForDept: string[];  // role filter when scope=dept ([] = All)
  locations: string[];
  rules: PolicyRule[];
  approvers: string[];
}
...
/* ─────────────────────────────────────────────────────────────
   DropdownList — overflow:auto so short lists have NO scrollbar.
   searchable=true adds a filter input at the top of the popover.
───────────────────────────────────────────────────────────── */
function DropdownList({
  ...
  // Reset query whenever the dropdown re-mounts (i.e. opens fresh)
  useLayoutEffect(() => {
    setQuery("");
      ...
      {searchable && (
        <div className="px-2 pt-2 pb-1">
          <div className="flex items-center gap-2 h-9 px-3 rounded-xl border border-gray-200 bg-gray-50 focus-within:border-[#03C3A6] focus-within:bg-white transition-colors">
            {/* Inline magnifier — no extra import needed */}
...
function MultiDropdown({ placeholder, values, onToggle, options, footer, isLoading = false, searchable = false }: {
  ...
  const triggerLabel = (() => {
    ...
    if (values.length === 0) return null; // show placeholder
    if (values.length === 1) {
...
/* ─────────────────────────────────────────────────────────────
   RuleCard — deletable, with type selector + fields
   Since guidelines aren't finalised, each card shows:
   • Rule type (fixed for now: spend_limit or receipt_requirement)
   • Amount field
   • Enforcement dropdown
   • Delete button (red trash, top right)
───────────────────────────────────────────────────────────── */
function RuleCard({
...
/* ─────────────────────────────────────────────────────────────
   Preview — modern compact design
   Categories: labelled chip row
   Scope:       icon + text summary line
   Rules:       compact table (type | amount | enforcement)
                scales cleanly to many rules, no icon overload
   Approvers:   pill row
───────────────────────────────────────────────────────────── */
function Preview({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-[#03C3A6]/25 bg-gradient-to-br from-[#03C3A6]/[0.06] to-white p-5">
        ...
        {/* Category chips — clearly labelled */}
      ...
      {/* ── Rules — compact table, scales to any number ── */}
      {rules.length > 0 && (
...
// Available rule types — expand when client provides more guidelines
...
export default function PolicyCreationModal({
  ...
  // Step 2 — Scope
  // 3 radio options matching the designer's screenshots
  const [scope, setScope] = useState<"all" | "role" | "dept">("all");
  const [selectedRoles,    setSelectedRoles]    = useState<string[]>([]);
  const [scopeDeptForRole, setScopeDeptForRole] = useState<string[]>([]);   // dept filter when scope=role
  const [selectedDepts,    setSelectedDepts]    = useState<string[]>([]);
  const [scopeRoleForDept, setScopeRoleForDept] = useState<string[]>([]);   // role filter when scope=dept
  ...
  const reset = () => {
    ...
    setSelectedRoles([]); setScopeDeptForRole([]); setSelectedDepts([]); setScopeRoleForDept([]);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      onSuccess?.({
        ...
        selectedRoles, selectedDepts, scopeDeptForRole, scopeRoleForDept,
  ...
  const scopeValid =
    categories.length > 0 &&
    (scope === "all" ||
      (scope === "role" && selectedRoles.length > 0) ||
      (scope === "dept" && selectedDepts.length > 0));
        ...
        <div className={`relative bg-white rounded-[1.75rem] shadow-2xl w-full flex flex-col ${
          ...
          {step === 1 && (
            <div className="p-9">
              ...
              <h2 className="text-[22px] font-semibold text-gray-900 mb-7">
                Policy Name
              </h2>
              <input autoFocus
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6 min-h-0" style={{ scrollbarWidth: "none" } as React.CSSProperties}>
                ...
                {step === 2 && (
                  <>
                    ...
                    {/* Who — 3-radio approach from designer */}
                    <div>
                      <FieldLabel>Who should this policy apply to?</FieldLabel>
                      <div className="space-y-2">
                        {/*
                          All Employees — no sub-fields. "All departments" is
                          implicit and showing it would be redundant (designer's note).
                        */}
                        <RadioRow value="all"
                          label="All Employees"
                          subLabel="Applies to every employee across the company"
                          checked={scope === "all"}
                          onChange={(v) => setScope(v as "all" | "role" | "dept")}
                        />

                        {/*
                          Specific employee role — role is the primary filter.
                          Department is secondary / optional (defaults to All).
                        */}
                        <RadioRow value="role"
                          label="Specific employee role"
                          subLabel="Target employees by their job title across departments"
                          checked={scope === "role"}
                          onChange={(v) => setScope(v as "all" | "role" | "dept")}
                        />
                        {scope === "role" && (
                          <div className="ml-7 space-y-3 pt-1">
                            ...
                            <div>
                              ...
                              <MultiDropdown
                                ...
                                values={scopeDeptForRole}
                                onToggle={(v) => setScopeDeptForRole((p) => p.includes(v) ? p.filter((x) => x !== v) : [...p, v])}
                                options={departmentOptions}
                                isLoading={departmentsApi.isLoading}
                                searchable
                              />
                              {scopeDeptForRole.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                  {scopeDeptForRole.map((d) => (
                                    <Chip key={d} label={departmentOptions.find((o) => o.value === d)?.label ?? d}
                                      onRemove={() => setScopeDeptForRole((p) => p.filter((x) => x !== d))} />
                                  ))}
                                </div>
                              )}
                            </div>
                        ...
                        {/*
                          Specific department — department is the primary filter.
                          Role is secondary / optional (defaults to All employees).
                          Order matches screenshot 3: Department first, then Role.
                        */}
                        <RadioRow value="dept"
                          label="Specific department"
                          subLabel="Target everyone — or a specific role — within a department"
                          checked={scope === "dept"}
                          onChange={(v) => setScope(v as "all" | "role" | "dept")}
                        />
                        {scope === "dept" && (
                          <div className="ml-7 space-y-3 pt-1">
                            <div>
                              ...
                              {selectedDepts.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                  {selectedDepts.map((d) => (
                                    <Chip key={d} label={departmentOptions.find((o) => o.value === d)?.label ?? d} onRemove={() => togDept(d)} />
                                  ))}
                                </div>
                              )}
                            </div>
                            <div>
                              ...
                              <MultiDropdown
                                ...
                                values={scopeRoleForDept}
                                onToggle={(v) => setScopeRoleForDept((p) => p.includes(v) ? p.filter((x) => x !== v) : [...p, v])}
                                options={roleOptions}
                                isLoading={rolesApi.isLoading}
                                searchable
                              />
                              {scopeRoleForDept.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                  {scopeRoleForDept.map((r) => (
                                    <Chip key={r} label={roleOptions.find((o) => o.value === r)?.label ?? r}
                                      onRemove={() => setScopeRoleForDept((p) => p.filter((x) => x !== r))} />
                                  ))}
                                </div>
                              )}
                            </div>
                    ...
                    {/* Location */}
                    <div>
                      <FieldLabel>Location Filter <span className="text-gray-400 font-normal text-xs">(Optional)</span></FieldLabel>
                      {locations.length > 0 && (
              ...
              <div className="px-8 py-5 shrink-0 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  {/* Save as Draft — visible on steps 2–4, not on preview */}
                  {step < 5 && (
